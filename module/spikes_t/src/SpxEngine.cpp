...
//=============== SLIC CUDA ===============
SlicEngine_CUDA::SlicEngine_CUDA(int diamSpx_or_Nspx, float wc, SlicCuda::InitType initType)
{
	slic_cuda = make_unique<SlicCuda>();
}


void SlicEngine_CUDA::Init(Mat& imBGR)
{
	slic_cuda->initialize(imBGR.size());
}

void SlicEngine_CUDA::Init(Mat& imBGR, int diamSpx_or_Nspx, float wc, Segmentor::InitType initType)
{
	SlicCuda::InitType it;
	if (initType == SIZE)it = SlicCuda::SLIC_SIZE;
	else it = SlicCuda::SLIC_NSPX;
	//slic_cuda->setParam(diamSpx_or_Nspx,wc,it);
	slic_cuda->initialize(imBGR.size(),diamSpx_or_Nspx, it,wc,5);
}
void SlicEngine_CUDA::Segment(Mat& imBGR)
{
    ...
    cm_nSpx = slic_cuda->enforceConnectivity();
}
